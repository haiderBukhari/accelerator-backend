...
export const getCourses = async (req, res) => {
    try {
        const courses = await CourseModel.find().lean(); // Use lean() for faster read-only query
...
export const uploadModule = async (req, res) => {
    try {
        ...
        const file1Url = `https://storage.googleapis.com/${bucket.name}/${file1.originalname}`;
